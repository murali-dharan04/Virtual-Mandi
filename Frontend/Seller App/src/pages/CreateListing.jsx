...
import { 
    ...
    Leaf, TrendingUp, Sparkles, X, Check
} from "lucide-react";
...
const CreateListing = () => {
    ...
    const handleSubmit = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await sellerApi.createListing({
                ...form,
                imageUrl: form.images[0] || "/placeholder.svg"
            });
            if (response.id || response._id || response.listing_id) {
                toast({ title: "Crop Added!", description: "Your crop is now live for buyers." });
                navigate("/listings");
            }
        } catch (err) {
        ...
        <PageTransition>
            <div className="max-w-4xl mx-auto pb-24 px-4 bg-[#f8f9fa] min-h-screen pt-4">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-3xl font-serif font-black text-[#1b4332] tracking-tight leading-none">Add New Listing</h1>
                        <p className="text-xs font-medium text-slate-500 mt-1">Fill in the details below to publish your crop.</p>
                    </div>
                </div>
